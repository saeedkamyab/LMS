...
{
    public class UpdatePhoneNumberRequestHandler : IRequestHandler<UpdatePhoneNumberRequest, BaseCommandResponse>
    {
        IUnitOfWork _unitOfWork;
        IMapper _mapper;
        PhoneNumberValidator _validator;
        public UpdatePhoneNumberRequestHandler(IUnitOfWork unitOfWork, IMapper mapper, PhoneNumberValidator validator)
        {
            _unitOfWork = unitOfWork;
            _mapper = mapper;
            _validator = validator;
        }
        public async Task<BaseCommandResponse> Handle(UpdatePhoneNumberRequest request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                await _unitOfWork.PhoneNumberRepository.UpdateAsync(PhoneNumber);
                return new BaseCommandResponse
                {
