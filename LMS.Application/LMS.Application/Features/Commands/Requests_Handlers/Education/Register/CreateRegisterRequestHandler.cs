...
namespace LMS.Application.Features.Commands.Requests_Handlers.Education.Register
{
    public class CreateRegisterRequestHandler : IRequestHandler<CreateRegisterRequest, BaseCommandResponse>
    {
        IUnitOfWork _unitOfWork;
        IMapper _mapper;
        RegisterValidator _validator;
        public CreateRegisterRequestHandler(IUnitOfWork unitOfWork, IMapper mapper, RegisterValidator validator)
        {
            _unitOfWork = unitOfWork;
            _mapper = mapper;
            _validator = validator;
        }
        public async Task<BaseCommandResponse> Handle(CreateRegisterRequest request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                await _unitOfWork.RegisterRepository.AddAsync(Register);
                return new BaseCommandResponse
                {
