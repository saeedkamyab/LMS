...
namespace LMS.Application.Features.Commands.Requests_Handlers.Finance.PaymentTuition
{
    public class UpdatePaymentTuitionRequestHandler : IRequestHandler<UpdatePaymentTuitionRequest, BaseCommandResponse>
    {
        IUnitOfWork _unitOfWork;
        IMapper _mapper;
        PaymentTuitionValidator _validator;
        public UpdatePaymentTuitionRequestHandler(IUnitOfWork unitOfWork, IMapper mapper, PaymentTuitionValidator validator)
        {
            _unitOfWork = unitOfWork;
            _mapper = mapper;
            _validator = validator;
        }
        public async Task<BaseCommandResponse> Handle(UpdatePaymentTuitionRequest request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                await _unitOfWork.PaymentTuitionRepository.UpdateAsync(PaymentTuition);
                return new BaseCommandResponse
                {
